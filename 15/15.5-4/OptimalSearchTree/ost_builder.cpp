    ...
    unordered_map<string*, float>& keyProbs, unordered_map<string*, float>& dummyKeyProbs)
{
    ...
    for (auto length = 1; length < keys.size(); length++)
    {
        for (auto startInterval = 1; startInterval <= keys.size() - length; startInterval++) {
            ...
            auto value = DBL_MAX;
            auto bestRoot =  -1;
            auto startRoot = (endInterval - startInterval > 0) 
                ? roots[startInterval][endInterval - 1]
                : startInterval;
            auto endRoot = (endInterval - startInterval > 0)
                ? roots[startInterval + 1][endInterval]
                : endInterval;
            for (auto candidateRoot = startRoot; candidateRoot <= endRoot; candidateRoot++) {
